    ...
    class BP_Better_Messages_Notifications
    {
        ...
        public function notifications_sender()
        {
            ...
            $unread_threads = $wpdb->get_results( "SELECT
              {$wpdb->usermeta}.meta_value AS last_visit,
            ...
            FROM " . bpbm_get_table('recipients') . "
              INNER JOIN {$wpdb->usermeta}
                ON " . bpbm_get_table('recipients') . ".user_id = {$wpdb->usermeta}.user_id
            ...
            foreach ( $unread_threads as $thread ) {
                ...
                if ( ! isset( $last_notified[ $user_id ][ $thread_id ] ) || ( $thread->last_id > $last_notified[ $user_id ][ $thread_id ] ) ) {
                    ...
                    $query = $wpdb->prepare( "
                        SELECT
                          " . bpbm_get_table('messages') . ".id,
                          " . bpbm_get_table('messages') . ".message,
                          " . bpbm_get_table('messages') . ".sender_id,
                          " . bpbm_get_table('messages') . ".subject,
                          " . bpbm_get_table('messages') . ".date_sent
                        FROM " . bpbm_get_table('messages') . "
                        WHERE " . bpbm_get_table('messages') . ".thread_id = %d
                        AND " . bpbm_get_table('messages') . ".id > %d 
                        AND " . bpbm_get_table('messages') . ".sender_id != %d 
                        AND " . bpbm_get_table('messages') . ".sender_id != 0 
                        ORDER BY id DESC
                        LIMIT 0, %d
                    ...
                    if( $email_overwritten === false ) {
                        ...
                        if (function_exists('bp_send_email')) {
                            ...
                            $subject = sprintf(_x('You have unread messages: "%s"', 'Email notification header for non BuddyPress websites', 'bp-better-messages'), BP_Better_Messages()->functions->remove_re(sanitize_text_field(stripslashes($messages[0]->subject))));
