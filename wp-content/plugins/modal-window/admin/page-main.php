...
if($action_popup === 'activate') {
	...
	$message_data = esc_attr__( 'Modal Window Activated', $this->plugin['text'] );
	echo '<div class="updated" id="message"><p><strong>' . esc_attr($message_data) . '</strong>.</p></div>';
} elseif ($action_popup === 'deactivate') {
	$id = absint( $_GET['id'] );
	$wpdb->update( $data,array( 'status' => '' ), array( 'ID' => $id ), array( '%d' )	);
	$message_data = esc_attr__( 'Modal Window Deactivated', $this->plugin['text'] );
	echo '<div class="updated" id="message"><p><strong>' . esc_attr($message_data) . '</strong>.</p></div>';
}
    ...
    <div class="wrap">
		...
		<?php if ( get_option( 'wow_' . $this->plugin['prefix'] . '_message' ) != 'read' ) : ?>
            <div class="notice notice-info is-dismissible wow-plugin-message">
                <p class="ideas">
                    <i class="dashicons dashicons-megaphone has-text-danger is-r-margin"></i>We are constantly trying to improve the plugin and add more useful
                    features to it. Your support and your ideas for improving the plugin are very important to us. <br/>
                    <i class="dashicons dashicons-star-filled has-text-warning is-r-margin"></i>If you like the plugin, please <a
                            href="<?php echo esc_url( $rating ); ?>" target="_blank">leave a review</a> about it at
                    WordPress.org.<br/>
                    <i class="dashicons dashicons-share has-text-info is-r-margin"></i>Help other users find this plugin and take advantage of it. <b>Share:</b>
		...
		echo '<a class="nav-tab" href="https://wow-company.com/preview/wordpress-plugins/wow-modal-windows-pro/" target="_blank">Demo Pro</a>';
		echo '</h2>';
