			...
			if( isset($setting_data['cf7_opt_is_enable']) && $setting_data['cf7_opt_is_enable'] == 'true'  )
			{
				...
				if( isset($setting_data['cf7_opt_is_attach_enable']) && $setting_data['cf7_opt_is_attach_enable'] == 'true'  )
					...
					if($attach_image)
	 				{
						...
						setcookie( 'wp-enable_pdf_link', $cf7_pdf_link_is_enable, time() + (86400 * 1), "/"); // 86400 = 1 day
						setcookie( 'wp-pdf_download_link_txt', $cf7_pdf_download_link_txt, time() + (86400 * 1), "/"); // 86400 = 1 day
	 			...
	 			else
	 			{
	 				...
	 				*/
	 				...
	 				require  WP_CF7_PDF_DIR . 'inc/lib/mpdf/vendor/autoload.php';
						...
						if(!$cf7_opt_margin_right){$cf7_opt_margin_right = '15';}
	 					$mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'A4', 'margin_header' => $cf7_opt_margin_header, 'margin_top' => $cf7_opt_margin_top,'margin_footer' => $cf7_opt_margin_footer, 'margin_bottom' => $cf7_opt_margin_bottom,'default_font' => 'FreeSans', 'pagenumPrefix' => 'Page','margin_left' => $cf7_opt_margin_left,'margin_right' => $cf7_opt_margin_right]);
	 				}
					...
					$current_time = time().microtime(true);
