		...
		if( isset($_POST['action']) )
		{
			if (($_FILES['wp_cf7_pdf_settings']['name']['cf7_opt_attach_pdf_image'] != "" )){
				...
				$file = preg_replace('/\s+/', '', $file);
	 			...
	 			$ext = $path['extension'];
	 			$temp_name = sanitize_text_field($_FILES['wp_cf7_pdf_settings']['tmp_name']['cf7_opt_attach_pdf_image']);
	 			$path_filename_ext = $target_dir.$filename.".".$ext;
	 			if (move_uploaded_file($temp_name,$path_filename_ext)) {
		 			$temp = 1;
		 			@unlink(realpath($target_dir.$_POST['wp_cf7_pdf_settings']['cf7_opt_attach_pdf_old_url']));
				   } else {
				   	$temp = 0;
			   	}
			}
		        	...
		        	<table class="enable-pdf-link">
		        		<tr valign="top">
							...
							<td>
								...
								?>
								<input type="radio" id="cf7_pdf_link_enable_yes" name="wp_cf7_pdf_settings[cf7_pdf_link_is_enable]" class="" value="true" <?php if( $cf7_pdf_link_is_enable == 'true' ) { echo ' checked'; } ?> />
								<label for="cf7_pdf_link_enable_yes"><?php echo esc_html(__( 'Yes', 'cf7-pdf-generation')); ?></label>
								
								<input type="radio" id="cf7_pdf_link_enable_no" name="wp_cf7_pdf_settings[cf7_pdf_link_is_enable]" class="" value="false" <?php if( $cf7_pdf_link_is_enable == 'false' ) { echo ' checked'; } ?> />
	        		...
	        		<table class="enable-pdf">
				    	...
				    	<tr>
					    	...
					    	<table class="pdf-genrate" style="display: none">
						        ...
						        <tr valign="top">
						        	<th scope="row">
									<?php echo esc_html(__( 'Message body', 'cf7-pdf-generation')); ?>
			...
			jQuery( '#cf7_pdf_msg_body_tooltip_id' ).on( 'mouseenter click', function() {
				...
				jQuery( '#cf7_pdf_msg_body_tooltip_id' ).pointer({
					...
					_e( '<h3>Message body</h3>' .
						'<p>You can manage body content of the message which will automatically reflect in the PDF attachement.</p><p>For the new page you can use &lt;pagebreak&gt; tag</p>',
						'cf7-pdf-generation'
					); ?>',
					position: 'left center',
				} ).pointer('open');
